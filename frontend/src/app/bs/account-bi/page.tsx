  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-2">
        <span style={{ fontSize: 20, fontWeight: 700, color: "#1A1D23" }}>BS 계정분석</span>
        <BiTag />
      ...
      <div className={PANEL_CLS} style={{ ...PANEL_STYLE, padding: "12px 16px" }}>
        <div className="flex items-center gap-4">
          <span style={{ fontSize: 13, fontWeight: 600, color: "#1A1D23" }}>비교대상</span>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead style={{ position: "sticky", top: 0, zIndex: 1 }}>
              <tr style={{ backgroundColor: "#F5F7F8" }}>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "#7A8290" }}>공시용계정</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "#7A8290" }}>구분</th>
                <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: "#7A8290" }}>기초(S)</th>
                <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: "#7A8290" }}>기말</th>
                <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: "#7A8290" }}>증감</th>
                <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: "#7A8290" }}>증감률</th>
              </tr>
            </thead>
            <tbody>
              {(detailRows as BsDetailRow[]).map((d, i) => {
                ...
                return (
                  <tr key={i}
                    ...
                    onMouseEnter={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.backgroundColor = "#FFE8D4"; }}
                    onMouseLeave={e => { if (!isSelected) (e.currentTarget as HTMLElement).style.backgroundColor = ""; }}>
                    <td style={{ padding: "7px 14px", color: "#1A1D23", fontWeight: isSelected ? 600 : 500 }}>{d.account}</td>
                    ...
                    <td style={{ padding: "7px 14px", textAlign: "right", fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>{d.opening.toLocaleString("ko-KR")}</td>
                    <td style={{ padding: "7px 14px", textAlign: "right", fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>{d.closing.toLocaleString("ko-KR")}</td>
                    <td style={{ padding: "7px 14px", textAlign: "right", color: d.delta >= 0 ? POS_COLOR : NEG_COLOR, fontWeight: 500, fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>
                      {d.delta.toLocaleString("ko-KR")}
                    </td>
                    <td style={{ padding: "7px 14px", textAlign: "right", color: d.delta >= 0 ? POS_COLOR : NEG_COLOR, fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>
            ...
            <div className={PANEL_CLS} style={PANEL_STYLE}>
              ...
              <div style={{ padding: "12px 24px 24px" }}>
                {dailyChartData.length > 0 ? (
                  ...
                  <div style={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center", color: "#7A8290", fontSize: 13 }}>
                    데이터를 불러오는 중...
                  </div>
                )}
              </div>
                ...
                <tbody>
                  {(entries as BsEntry[]).length === 0 ? (
                    ...
                    <tr key={i} style={{ borderTop: "1px solid #EEEFF1" }}
                      ...
                      <td style={{ padding: "6px 10px", textAlign: "right", color: e.debit > 0 ? POS_COLOR : "#4A5056", fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>
                        {e.debit > 0 ? e.debit.toLocaleString("ko-KR") : ""}
                      </td>
                      <td style={{ padding: "6px 10px", textAlign: "right", color: e.credit > 0 ? NEG_COLOR : "#4A5056", fontVariantNumeric: "tabular-nums", fontFeatureSettings: "'tnum' 1, 'zero' 1" }}>
